    ...
    for (int v = 0; v < n; ++v) {
        ...
        if (cmd == (int)VoiceCmd::Type::note_on) {
            ...
            if (!is_glide) {
                ...
                if (do_declick) {
                    // Always declick. We want the retrigger stage regardless of whether
                    // the voice is active or releasing. Force m_active=true by triggering
                    // a silent note-on first if needed, so trigger(true) always goes to
                    // the retrigger stage rather than straight to attack.
                    applyADSRTimingOnly(v);
                    if (!voice_adsr[v].active()) {
                        // Voice is releasing (m_active=false) — trigger(true) would skip
                        // the retrigger stage. Force it by calling trigger(true) to activate,
                        // then trigger(true) again to enter retrigger.
                    ...
                    // retrigger stage always active now — defer freq/peak/sustain to bottom
