...
void operator()(audio_bundle input, audio_bundle output) {
    ...
    for (int v = 0; v < n; ++v) {
        ...
        int cmd = voice_cmd[v].cmd.load(std::memory_order_acquire);
       ...
       // Output channel base offsets (9 MC outlets × n channels each)
        ...
        voice_cmd[v].cmd.store(0, std::memory_order_release);
    }
...
timer<timer_options::defer_delivery> adsr_poll{this, MIN_FUNCTION{
    const int n = voices;
    for (int v = 0; v < n; ++v) {
        // 1. CHECK COMMAND FIRST: If the audio thread hasn't finished processing 
        // a new command, skip. Do NOT touch the done flag yet.
        if (voice_cmd[v].cmd.load(std::memory_order_acquire) != 0) continue;

        // 2. Now it is safe to consume the done flag.
        ...
        bool already_inactive = std::find(inactive_voices.begin(), inactive_voices.end(), target) != inactive_voices.end();
        if (!already_inactive)
...
void handleNoteOnMono(Note& note, lock& lock) {
    ...
    if (s) {
        ...
        nts = active_voices.back(); lock.unlock();
        // Send note-off message for stolen voice so poly~ doesn't get stuck,
        // but don't write to voice_cmd — the steal declick handles the audio transition.
        out_msg.send("target", old_note.target);
        out_msg.send("notes", (float)old_note.freq.back(), 0, "flags",
                     0, (int)old_note.hold_flag, (int)old_note.sustain_flag,
                     (int)old_note.sequencer_note_flag, (int)old_note.mono_flag, old_note.stream);
        outputFunction(nts, 1, 1);
    }
...
void handleNoteOnPoly(Note& note, lock& lock) {
    ...
    if (fv != -1) {
        ...
        Note* s = findNoteToSteal(note);
        if (s) {
            // Capture old note for note-off before overwriting
            Note old_note = *s;
            old_note.release_flag = true;
            s->mpitch = note.mpitch; s->freq = note.freq; s->vel = note.vel;
            s->stream = note.stream; s->sequencer_note_flag = note.sequencer_note_flag;
            ...
            size_t idx = s - &active_voices[0];
            rotate(active_voices.begin()+idx, active_voices.begin()+idx+1, active_voices.end());
            Note new_note = active_voices.back();
            lock.unlock();
            // Send note-off message for stolen voice so poly~ doesn't get stuck,
            // but don't write to voice_cmd — the steal declick handles the audio transition.
            out_msg.send("target", old_note.target);
...
void outputFunction(const Note note, bool note_on, bool steal, bool flags_only = false, bool glide_note = false) {
    ...
    if (note_on && !flags_only) {
        ...
        store_flags();
        voice_cmd[vi].pending_off.store(0, std::memory_order_relaxed); //maybe?
        ...
        if (it != pending_voices.end()) {
            ...
            // Voice should be in active_voices (steal/glide case) or pending (new note)
            bool in_active = std::any_of(active_voices.begin(), active_voices.end(),
                                         [&](const Note& n){ return n.target == note.target; });
            bool in_inactive = std::find(inactive_voices.begin(), inactive_voices.end(), note.target) != inactive_voices.end();
            if (!in_active && !in_inactive)
                cout << "WARNING: note-on for voice " << note.target << " not found in any collection!" << endl;
        }
    }
...
message<> print{this, "print", "Print active voices and all parameter settings to console",
    MIN_FUNCTION{
        ...
        if (pending_voices.empty()) cout << "No notes in pending_voices" << endl;
        else {
            cout << pending_voices.size() << " pending note(s){" << endl;
            for (auto& kv : pending_voices)
                cout << "  target=" << kv.first << " mpitch=" << kv.second.mpitch.back()
                     << " release=" << kv.second.release_flag << endl;
            cout << "}" << endl;
        }
        cout << inactive_voices.size() << " inactive voice(s)" << endl;
        cout << "total accounted: " << active_voices.size() + pending_voices.size() + inactive_voices.size()
             << " / " << voices << endl;
