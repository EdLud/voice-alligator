...
// Per-inlet MC channel counts — set by user via attributes or multichannelinputs callback
std::atomic<int> inlet_trig_chans {1};
std::atomic<int> inlet_pitch_chans{1};
std::atomic<int> inlet_vel_chans  {1};

// Previous trigger signal per channel — for rising edge detection (audio thread)
float prev_trigger_samples[1024] {};
...
attribute<int> trig_chans_attr{this, "trig_chans", 1,
    description{"Number of channels connected to inlet 1 (trigger). "
                "Required when using MC signals so the object knows how to split the input bundle."},
    setter{MIN_FUNCTION{
        inlet_trig_chans.store(std::max(1, (int)args[0]), std::memory_order_relaxed);
        return args;
    }}
};

attribute<int> pitch_chans_attr{this, "pitch_chans", 1,
    description{"Number of channels connected to inlet 2 (pitch/frequency)."},
    setter{MIN_FUNCTION{
        inlet_pitch_chans.store(std::max(1, (int)args[0]), std::memory_order_relaxed);
        return args;
    }}
};

attribute<int> vel_chans_attr{this, "vel_chans", 1,
    description{"Number of channels connected to inlet 3 (velocity)."},
    setter{MIN_FUNCTION{
        inlet_vel_chans.store(std::max(1, (int)args[0]), std::memory_order_relaxed);
        return args;
    }}
};
...
void resetVoices(int n) {
    ...
    std::fill(std::begin(prev_trigger_samples), std::end(prev_trigger_samples), 0.f);
...
void operator()(audio_bundle input, audio_bundle output) {
    ...
    // ── Per-inlet channel counts ───────────────────────────────────────────────
    const int total_in  = static_cast<int>(input.channel_count());
    int trig_chans  = std::max(1, (int)inlet_trig_chans .load(std::memory_order_relaxed));
    int pitch_chans = std::max(1, (int)inlet_pitch_chans.load(std::memory_order_relaxed));
    int vel_chans   = std::max(1, (int)inlet_vel_chans  .load(std::memory_order_relaxed));
    // Hard clamp — never read beyond actual bundle
    trig_chans  = std::min(trig_chans,  total_in);
    pitch_chans = std::min(pitch_chans, std::max(0, total_in - trig_chans));
    vel_chans   = std::min(vel_chans,   std::max(0, total_in - trig_chans - pitch_chans));

    // ── Rising-edge detection → post to poll timer via fifo ──────────────────
    for (int i = 0; i < frames; ++i) {
        for (int tc = 0; tc < trig_chans && tc < 1024; ++tc) {
            float trig = (float)input.samples(tc)[i];
            float prev = prev_trigger_samples[tc];

            if (trig != 0.f && prev == 0.f) {
                // Pitch: loop over pitch channels (fallback to default if none)
                float pitch_val = default_freq.load(std::memory_order_relaxed);
                if (pitch_chans > 0) {
                    int pitch_ch = trig_chans + (tc % pitch_chans);
                    float sig = (float)input.samples(pitch_ch)[i];
                    if (sig != 0.f) pitch_val = sig;
                }

                // Vel: loop over vel channels (fallback to default if none)
                float vel_norm = default_vel.load(std::memory_order_relaxed);
                if (vel_chans > 0) {
                    int vel_ch = trig_chans + pitch_chans + (tc % vel_chans);
                    float sig = (float)input.samples(vel_ch)[i];
                    if (sig != 0.f) vel_norm = sig;
                }
...
message<> dspsetup{this, "dspsetup", MIN_FUNCTION{
    ...
    // Per-inlet channel counts are derived in operator() from total_in and the
    // multichannelinputs callback (if it fires). Store total_in at dspsetup for
    // validation. The atomics default to 1/1/1 until multichannelinputs fires.
    if (debug) cout << "dspsetup: trig=" << inlet_trig_chans.load()
                    << " pitch=" << inlet_pitch_chans.load()
                    << " vel=" << inlet_vel_chans.load() << endl;
    {
        lock l{m_mutex};
        resetVoices(voices);
    }
...
message<threadsafe::yes> print_msg{this, "print",
    ...
    MIN_FUNCTION{
        ...
        cout << "  trig_chans          = " << inlet_trig_chans.load()  << endl;
        cout << "  pitch_chans         = " << inlet_pitch_chans.load() << endl;
        cout << "  vel_chans           = " << inlet_vel_chans.load()   << endl;
...
// Called by Max when the MC signal graph changes — tells us how many channels
// are actually connected to each inlet. We store these so the audio thread
// can correctly split the input bundle without guessing.
long voice_alligator_audio_tilde_multichannelinputs(c74::max::t_object* x, long index, long count) {
    minwrap<voice_alligator_audio_tilde>* ob = (minwrap<voice_alligator_audio_tilde>*)(x);
    auto& obj = ob->m_min_object;
    int chans = (count > 0) ? (int)count : 1;
    if      (index == 0) obj.inlet_trig_chans .store(chans, std::memory_order_relaxed);
    else if (index == 1) obj.inlet_pitch_chans.store(chans, std::memory_order_relaxed);
    else if (index == 2) obj.inlet_vel_chans  .store(chans, std::memory_order_relaxed);
    c74::max::object_post(x, "multichannelinputs: inlet=%ld count=%ld -> chans=%d", index, count, chans);
    return chans;
}
