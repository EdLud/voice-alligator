...
// Per-inlet MC channel counts — updated by multichannelinputs callback
std::atomic<int> inlet_trig_chans {1};
std::atomic<int> inlet_pitch_chans{1};
std::atomic<int> inlet_vel_chans  {1};

// Per-inlet channel counts captured at dspsetup time (audio-thread read-only)
int dsp_trig_chans  {1};
int dsp_pitch_chans {1};
int dsp_vel_chans   {1};
...
void operator()(audio_bundle input, audio_bundle output) {
    ...
    // Primary source: multichannelinputs callback. Validated against the actual
    // bundle size — if they don't match (callback not fired yet), derive from
    // dspsetup-time channel counts stored below.
    const int total_in    = static_cast<int>(input.channel_count());
    int trig_chans  = inlet_trig_chans .load(std::memory_order_relaxed);
    int pitch_chans = inlet_pitch_chans.load(std::memory_order_relaxed);
    int vel_chans   = inlet_vel_chans  .load(std::memory_order_relaxed);
    if (trig_chans + pitch_chans + vel_chans != total_in) {
        // Callback hasn't fired or gave wrong values — use dspsetup counts
        trig_chans  = dsp_trig_chans;
        pitch_chans = dsp_pitch_chans;
        vel_chans   = dsp_vel_chans;
    }
    ...
    for (int i = 0; i < frames; ++i) {
        for (int tc = 0; tc < trig_chans && tc < 1024; ++tc) {
            ...
            if (trig != 0.f && prev == 0.f) {
                // Pitch: loop over pitch channels
                int   pitch_ch  = trig_chans + (tc % pitch_chans);
                float pitch_val = (float)input.samples(pitch_ch)[i];

                // Vel: loop over vel channels
                int   vel_ch   = trig_chans + pitch_chans + (tc % vel_chans);
                float vel_norm = (float)input.samples(vel_ch)[i];
...
message<> dspsetup{this, "dspsetup", MIN_FUNCTION{
    ...
    // args layout from Min dspsetup: [sample_rate, in_ch_inlet0, in_ch_inlet1, in_ch_inlet2, ...]
    // Capture per-inlet channel counts for use in operator() validation.
    dsp_trig_chans  = (args.size() > 1) ? std::max(1, (int)args[1]) : 1;
    dsp_pitch_chans = (args.size() > 2) ? std::max(1, (int)args[2]) : 1;
    dsp_vel_chans   = (args.size() > 3) ? std::max(1, (int)args[3]) : 1;
    if (debug) cout << "dspsetup: trig=" << dsp_trig_chans
                    << " pitch=" << dsp_pitch_chans
                    << " vel=" << dsp_vel_chans
                    << " total_args=" << args.size() << endl;
    {
        lock l{m_mutex};
        resetVoices(voices);
    }
...
long voice_alligator_audio_tilde_multichannelinputs(c74::max::t_object* x, long index, long count) {
    ...
    // if ((bool)obj.debug) cout << "multichannelinputs: inlet=" << index
    //                           << " count=" << count << " → chans=" << chans << endl;
    return chans;
}
