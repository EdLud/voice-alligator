...
long voice_alligator_audio_multichanneloutputs(c74::max::t_object* x, long index, long count);

class voice_alligator_audio : public object<voice_alligator_audio>, public vector_operator<> {
public:
...
// Single message inlet — no ADSR feedback inlet needed
inlet<> in1{this, "(list) midipitch, velocity, (stream), (mono_flag), (realpitch)"};

// Eight MC signal outlets
outlet<> out_freq   {this, "(MC~) frequency per voice",           "multichannelsignal"};
outlet<> out_env    {this, "(MC~) ADSR envelope per voice",       "multichannelsignal"};
outlet<> out_glide  {this, "(MC~) glide flag per voice",          "multichannelsignal"};
outlet<> out_hold   {this, "(MC~) hold flag per voice",           "multichannelsignal"};
outlet<> out_sustain{this, "(MC~) sustain flag per voice",        "multichannelsignal"};
outlet<> out_seq    {this, "(MC~) sequencer_note flag per voice", "multichannelsignal"};
outlet<> out_mono   {this, "(MC~) mono flag per voice",           "multichannelsignal"};
outlet<> out_impulse{this, "(MC~) non-glide note-on impulse",     "multichannelsignal"};
outlet<> out_msg   {this, "messages to poly~ object, [(notes), freq, velocity, (flags), glide, hold, sustain, sequencer, mono, stream]"};

message<> maxclass_setup{this, "maxclass_setup", MIN_FUNCTION{
    c74::max::t_class* c = args[0];
    c74::max::class_addmethod(c, (c74::max::method)voice_alligator_audio_multichanneloutputs,
...
struct VoiceCmd {
    ...
    std::atomic<int>   impulse  {0};   // 1 = fire 1-sample impulse next vector
...
std::atomic<bool>  adsr_params_dirty  {false};
...
std::atomic<bool>  glide_retrigger{false}; // if false, glide notes skip ADSR retrigger (default)
...
attribute<number> glidetime_attr{this, "glidetime", 0.0,
    description{"Glide time in milliseconds. 0 = no glide (default 0)"},
    setter{MIN_FUNCTION{
...
attribute<bool> glide_retrigger_attr{this, "glide_retrigger", false,
    description{"If false (default), glide notes do not retrigger the ADSR — the envelope continues uninterrupted. If true, glide notes retrigger the ADSR as normal."},
    setter{MIN_FUNCTION{
...
attribute<number> attack_attr{this, "attack", 10.0,
    ...
    setter{MIN_FUNCTION{
        ...
        adsr_params_dirty.store(true, std::memory_order_relaxed);
        return args;
    }}
};
...
attribute<number> retrigger_ms_attr{this, "retrigger_ms", 5.0,
    description{"ADSR retrigger ramp time in milliseconds (default 5)"},
    setter{MIN_FUNCTION{
...
attribute<bool> return_to_zero_attr{this, "return_to_zero", true,
    description{"ADSR retrigger returns to zero before attack (default true)"},
    setter{MIN_FUNCTION{
...
voice_alligator_audio(const atoms& args = {}) {
    setStealcase();
    int n = voices;
...
void applyADSRParams(int v) {
    ...
    voice_adsr[v].sustain       (adsr_sustain_lvl .load(std::memory_order_relaxed));
...
void operator()(audio_bundle input, audio_bundle output) {
    ...
    // Apply any pending ADSR parameter changes to all voices
    if (adsr_params_dirty.exchange(false, std::memory_order_relaxed)) {
        for (int v = 0; v < n; ++v) applyADSRParams(v);
    }
    ...
    for (int v = 0; v < n; ++v) {
        ...
        if (cmd == (int)VoiceCmd::Type::note_on) {
            ...
            if (is_glide && gt_samps > 0 && voice_glide[v].current_freq > 0.f) {
                // Start a glide from wherever we currently are
                ...
                // Non-glide or first note: snap immediately
                voice_glide[v].current_freq  = new_freq;
                voice_glide[v].target_freq   = new_freq;
                voice_glide[v].active        = false;
            }
            live_freq   [v] = new_freq; // target (used after glide completes)
            ...
            // Only trigger the ADSR if this is not a glide note, or if glide_retrigger is on
            if (!is_glide || glide_retrigger.load(std::memory_order_relaxed)) {
                applyADSRParams(v);
        ...
        // Output channel base offsets (8 outlets × n channels each)
        auto* ch_freq    = output.samples(0*n + v);
        auto* ch_env     = output.samples(1*n + v);
        auto* ch_glide   = output.samples(2*n + v);
        auto* ch_hold    = output.samples(3*n + v);
        auto* ch_sustain = output.samples(4*n + v);
        auto* ch_seq     = output.samples(5*n + v);
        auto* ch_mono    = output.samples(6*n + v);
        auto* ch_impulse = output.samples(7*n + v);
        ...
        for (int i = 0; i < frames; ++i) {
            ...
            if (audio_voice_active[v] && voice_adsr[v].stage() == adsr::adsr_stage::inactive) {
    ...
    if (!inc.sequencer_note_flag) {
        if (hold_attr && !sustain_attr) {
            if (auto* n = findFirstNoteWithPredicate([=](const Note& x){
                    return x.mpitch.back()==mpit && !x.hold_flag && x.stream==st && !x.release_flag; })) {
                n->hold_flag = true; nts = *n; lock.unlock();
                outputFunction(nts, 0, 0, true); return;
            }
        } else if (sustain_attr) {
            if (auto* n = findFirstNoteWithPredicate([=](const Note& x){
                    return x.mpitch.back()==mpit && !x.sustain_flag && x.stream==st && !x.release_flag; })) {
...
message<> print{this, "print", "Print active voices to console",
    MIN_FUNCTION{
...
// All 8 outlets carry `voices` channels each.

long voice_alligator_audio_multichanneloutputs(c74::max::t_object* x, long index, long count) {
    minwrap<voice_alligator_audio>* ob = (minwrap<voice_alligator_audio>*)(x);
    return ob->m_min_object.voices;
}

MIN_EXTERNAL(voice_alligator_audio);
