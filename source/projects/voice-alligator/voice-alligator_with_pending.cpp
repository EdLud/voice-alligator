...
std::unordered_map<int, Note> pending_voices; // target -> Note, waiting for fromPoly muteflag==0
...
void handleNoteOnMono(Note &note, lock &lock){
    ...
    if (free_voice){
        // Don't pop yet — voice stays in inactive_voices until fromPoly confirms it started (muteflag==0)
        note.target = free_voice;
        note.mono_flag = true;
        pending_voices[free_voice] = note;
        if(debug){cout << "Found inactive voice with target " << free_voice << " and added new mono note to pending_voices (waiting for fromPoly muteflag==0)" << endl;}
        lock.unlock();
...
void handleNoteOnPoly(Note &note, lock &lock){
    ...
    if (free_voice){
        ...
        pending_voices[free_voice] = note;
        if(debug){cout << "Found inactive voice with target " << free_voice << " and added new note with mpitch " << note.mpitch.back() << " to pending_voices (waiting for fromPoly muteflag==0)" << endl;}
        lock.unlock();
...
// Returns the first inactive voice that is not already waiting in pending_voices, or 0 if none.
int findFreeVoice(){
    for (int candidate : inactive_voices){
        if (pending_voices.find(candidate) == pending_voices.end())
            return candidate;
    }
    return 0;
}
...
void fromPoly(const int target, const int muteflag, const lock &lock){
    ...
    if (muteflag == 0){
        // Voice has started — pop from inactive_voices, promote from pending to active
        auto it = pending_voices.find(target);
        if (it != pending_voices.end()){
            if(debug){cout << "fromPoly muteflag==0: promoting target " << target << " from pending_voices to active_voices" << endl;}
            // Remove this target from inactive_voices (it is now truly active)
        ...
        // (if target wasn't pending, it's a status report for an already-active voice — ignore)
    }
    else{
        // Voice has stopped — remove from active_voices and return target to pool
        ...
        if (auto *note = findFirstNoteWithPredicate([=](const Note &n)
            ...
            // Also clean up if it was still pending (e.g. voice died before confirming start)
            auto it = pending_voices.find(target);
            if (it != pending_voices.end()){
                if(debug){cout << "fromPoly muteflag!=0: removing pending note at target " << target << endl;}
                inactive_voices.push_back(target);
                pending_voices.erase(it);
            }
        }
