...
class mc_voice_alligator : public object<voice_alligator>, public vector_operator<> {
public:

    // --- MC signal buffers ---
    std::vector<double> freq_signal;
    std::vector<double> vel_signal;
    std::vector<double> glide_signal;
    std::vector<double> hold_signal;
    std::vector<double> sustain_signal;
    std::vector<double> seq_signal;
    std::vector<double> mono_signal;
    std::vector<double> stream_signal;
    std::vector<std::atomic<double>> trig_signal;
...
outlet<> freq_out{this, "(multichannelsignal) frequency", "multichannelsignal"};
outlet<> vel_out{this, "(multichannelsignal) velocity", "multichannelsignal"};
outlet<> glide_out{this, "(multichannelsignal) glide", "multichannelsignal"};
outlet<> hold_out{this, "(multichannelsignal) hold", "multichannelsignal"};
outlet<> sustain_out{this, "(multichannelsignal) sustain", "multichannelsignal"};
outlet<> seq_out{this, "(multichannelsignal) sequencer", "multichannelsignal"};
outlet<> mono_out{this, "(multichannelsignal) mono", "multichannelsignal"};
outlet<> stream_out{this, "(multichannelsignal) stream", "multichannelsignal"};
outlet<> trig_out{this, "(multichannelsignal) trigger", "multichannelsignal"};
...
void outputFunction(const Note note, bool note_on, bool steal, bool flags_only = false, bool glide_note = false){
    ...
    if (note_on){
        ...
        int v = note.target - 1;
        if(v >= 0){
            freq_signal.resize(std::max(freq_signal.size(), (size_t)(v+1)));
            vel_signal.resize(freq_signal.size());
            glide_signal.resize(freq_signal.size());
            hold_signal.resize(freq_signal.size());
            sustain_signal.resize(freq_signal.size());
            seq_signal.resize(freq_signal.size());
            mono_signal.resize(freq_signal.size());
            stream_signal.resize(freq_signal.size());
            trig_signal.resize(freq_signal.size());

            freq_signal[v] = note.freq.back();
            vel_signal[v] = note_on ? note.vel : 0.0;
            glide_signal[v] = glide_note;
            hold_signal[v] = note.hold_flag;
            sustain_signal[v] = note.sustain_flag;
            seq_signal[v] = note.sequencer_note_flag;
            mono_signal[v] = note.mono_flag;
            stream_signal[v] = note.stream;

            if(note_on && !glide_note)
                trig_signal[v].store(1.0);
        }
        // 
        if(!flags_only) {
...
MIN_EXTERNAL(mc_voice_alligator);