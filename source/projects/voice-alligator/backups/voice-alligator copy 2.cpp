...
voice_alligator(const atoms& args = {}){
            ...
            for (int i = 0; i < 1024; ++i) {
                adsr_start[i].store(false, std::memory_order_relaxed);
                adsr_end[i].store(false, std::memory_order_relaxed);
            }
...
std::atomic<bool> adsr_start[1024] {};
std::atomic<bool> adsr_end[1024] {};
...
void operator()(audio_bundle input, audio_bundle output){
    ...
    for (int v = 0; v < num_voices; ++v){
        ...
        if (!was_active && went_active) {
            adsr_start[v].store(true, std::memory_order_release);
        }
        if (was_active && !still_active) {
            adsr_end[v].store(true, std::memory_order_release);
        }
        // Special case: whole life in one vector – set both
...
timer<timer_options::deliver_on_scheduler> adsr_poll{this, MIN_FUNCTION{
    const int n = voices;                     // number of active voices
    for (int v = 0; v < n; ++v) {
        const int target = v + 1;              // 1‑based target number

        // Atomically read and clear both flags
        bool started = adsr_start[v].exchange(false, std::memory_order_acquire);
        bool ended   = adsr_end[v].exchange(false, std::memory_order_acquire);

        if (!started && !ended)
            continue;                           // nothing to do for this voice

        lock lock{m_mutex};                     // protect shared containers

        // --- ADSR START event ---
        if (started) {
            auto pit = pending_voices.find(target);
            if (pit != pending_voices.end()) {
                // Note was pending: promote it to active_voices
                if (debug)
                    cout << "ADSR start for target " << target
                         << " → promoting pending note to active" << endl;

                // Remove target from inactive_voices if it's still there (shouldn't be, but safe)
                auto vit = std::find(inactive_voices.begin(), inactive_voices.end(), target);
                if (vit != inactive_voices.end())
                    inactive_voices.erase(vit);

                // Move the pending note into active_voices
                active_voices.push_back(pit->second);
                pending_voices.erase(pit);
            }
            else {
                // A start without a pending note can happen if the voice was stolen:
                // the new note is already in pending_voices under the same target,
                // but the old note is still in active_voices. The old note will be
                // removed by the coming end event. Nothing else to do.
                if (debug)
                    cout << "ADSR start for target " << target
        ...
        if (ended) {
            ...
            if (ait != active_voices.end()) {
                // Normal case: finished note is in active_voices
                if (debug)
                    cout << "ADSR end for target " << target
                         << " → moving voice back to inactive pool" << endl;
                inactive_voices.push_back(target);
                active_voices.erase(ait);
            }
            else {
                // Possibly the note was still pending and had release_flag set
                auto pit = pending_voices.find(target);
                ...
                // If we still don't find it, the voice was already freed – ignore.
            }
        }
    }

    // Reschedule the timer to run again as soon as possible
    adsr_poll.delay(0);
    return {};
}};
