...
enum class VoiceEvent { NONE, START, END };

struct VoiceEventEntry {
    int target;          // 1‑based voice number
    VoiceEvent event;
};
...
voice_alligator(const atoms& args = {}){
            ...
            for (int i = 0; i < 1024; ++i) {
               event_write_index.store(0, std::memory_order_relaxed);
            event_read_index.store(0, std::memory_order_relaxed);
            }
...
void operator()(audio_bundle input, audio_bundle output) {
    ...
    for (int v = 0; v < num_voices; ++v) {
        ...
        const bool went_active = (peak > ADSR_THRESHOLD);           // fired at any point
        const bool still_active = (last > ADSR_THRESHOLD);          // still on at end

        const int target = v + 1;   // 1‑based voice number

        if (!was_active && went_active && !still_active) {
            // Entire note life within one vector – push both events
            push_event(target, VoiceEvent::START);
            push_event(target, VoiceEvent::END);
        }
        else {
            if (!was_active && went_active) {
                push_event(target, VoiceEvent::START);
            }
...
timer<timer_options::deliver_on_scheduler> adsr_poll{this, MIN_FUNCTION{
    ...
    // Optional: report dropped events occasionally
    long long drops = dropped_events.exchange(0, std::memory_order_relaxed);
    if (drops > 0 && debug) {
        cout << "Dropped " << drops << " ADSR events!" << endl;
    }

    int read = event_read_index.load(std::memory_order_acquire);
    int write = event_write_index.load(std::memory_order_acquire);

    while (read != write) {
        const auto& entry = event_queue[read];
        const int target = entry.target;

        switch (entry.event) {
            case VoiceEvent::START: {
                // Before promoting, remove any existing active note with the same target
                ...
                if (ait != active_voices.end()) {
                    if (debug)
                        cout << "ADSR start for target " << target
                             << " but active note already exists; removing it." << endl;
                    active_voices.erase(ait);
                    // Do NOT add target to inactive_voices – the voice is still playing
                }

                auto pit = pending_voices.find(target);
                if (pit != pending_voices.end()) {
                    // Promote pending note
                    if (debug)
                        cout << "ADSR start for target " << target
                             << " → promoting pending note to active" << endl;
                    auto vit = std::find(inactive_voices.begin(), inactive_voices.end(), target);
                    if (vit != inactive_voices.end())
                        inactive_voices.erase(vit);
                    active_voices.push_back(pit->second);
                    pending_voices.erase(pit);
                }
                else {
                    if (debug)
                        cout << "ADSR start for target " << target
                             << " but no pending note (probably a stolen voice)" << endl;
                }
                break;
            }
            ...
            case VoiceEvent::END: {
                ...
                if (ait != active_voices.end()) {
                    if (debug)
                        cout << "ADSR end for target " << target
                             << " → moving voice back to inactive pooly" << endl;
                    inactive_voices.push_back(target);
                    active_voices.erase(ait);
                }
                else {
                    auto pit = pending_voices.find(target);
                    if (pit != pending_voices.end() && pit->second.release_flag) {
                        if (debug)
                            cout << "ADSR end for target " << target
                                 << " (pending with release_flag) → discarding pending note" << endl;
                        pending_voices.erase(pit);
                    }
                }
                break;
        ...
        read = (read + 1) & (EVENT_QUEUE_SIZE - 1);
    }

    event_read_index.store(read, std::memory_order_release);

    static auto last_cleanup = std::chrono::steady_clock::now();
    auto now = std::chrono::steady_clock::now();
    if (now - last_cleanup > std::chrono::milliseconds(100)) {
        for (auto it = pending_voices.begin(); it != pending_voices.end(); ) {
            if (it->second.release_flag && (now - it->second.pending_since > std::chrono::milliseconds(200))) {
                if (debug) cout << "Cleaning up stale pending note on target " << it->first << endl;
                it = pending_voices.erase(it);
            } else {
                ++it;
            }
        }
...
void handleNoteOnMono(Note &note, lock &lock){
    ...
    // CASE: NO FREE VOICE AVAILABLE, LOOK FOR STEALING CANDIDATES
        ...
        if (noteToSteal) {
            if (debug) {
                cout << "** found note to steal on target " << noteToSteal->target << " **\n";
            }

            // Create a new note for the stolen target
            Note stolenNote = note;
            stolenNote.target = noteToSteal->target;
            stolenNote.mono_flag = true;
            stolenNote.release_flag = false;
            stolenNote.hold_flag = false;
            stolenNote.sustain_flag = false;

            pending_voices[stolenNote.target] = stolenNote;

            // Remove the old note from active_voices entirely
            size_t index = noteToSteal - &active_voices[0];
            active_voices.erase(active_voices.begin() + index);

            if (debug) {
                cout << "** created pending note for target " << stolenNote.target
                    << " and removed old note from active_voices **\n";
            }

            lock.unlock();
            outputFunction(stolenNote, 1, 1);
            return;
        }
...
if (noteToSteal) {
            ...
            // Remove the old active note with the same target
            auto existing = std::find_if(active_voices.begin(), active_voices.end(),
                                        [target = noteToSteal->target](const Note& n) { return n.target == target; });
            if (existing != active_voices.end()) {
                active_voices.erase(existing);
            }

            // Create a new note for the stolen target
            Note stolenNote = note;
            stolenNote.target = noteToSteal->target;
            stolenNote.mono_flag = false;
            stolenNote.release_flag = false;
            stolenNote.hold_flag = false;
            stolenNote.sustain_flag = false;
            stolenNote.pending_since = std::chrono::steady_clock::now();

            // Place the new note in pending_voices
            pending_voices[stolenNote.target] = stolenNote;
...
int findFreeVoice(){
    for (int candidate : inactive_voices){
        ...
        if (it == pending_voices.end())
            return candidate; // truly free
        // otherwise, candidate has a pending note – not free
    }
    return -1;
}
...
static constexpr int EVENT_QUEUE_SIZE = 8192; // must be power of two
std::atomic<int> event_write_index{0};
std::atomic<int> event_read_index{0};
std::atomic<long long> dropped_events{0};      // count of events dropped due to full queue
VoiceEventEntry event_queue[EVENT_QUEUE_SIZE];

void push_event(int target, VoiceEvent ev) {
    int write = event_write_index.load(std::memory_order_relaxed);
    int next_write = (write + 1) & (EVENT_QUEUE_SIZE - 1);
    if (next_write == event_read_index.load(std::memory_order_acquire)) {
        dropped_events.fetch_add(1, std::memory_order_relaxed);
        return;
    }
    event_queue[write].target = target;
    event_queue[write].event = ev;
    event_write_index.store(next_write, std::memory_order_release);
}
